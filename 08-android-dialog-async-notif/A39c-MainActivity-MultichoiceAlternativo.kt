...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btCheckBox.setOnClickListener {
            ...
            builder.setPositiveButton(android.R.string.ok) {  dialogo, _ ->
                val cantidadEscogidos = (dialogo as AlertDialog).listView.checkedItemCount
                val selecciones = (dialogo as AlertDialog).listView.checkedItemPositions
                var respuesta = "$cantidadEscogidos escogidos: "
                for (i in 0 .. selecciones.size()) {
                    val activado = selecciones[i]
                    if (activado)
                        respuesta += datos[i] + " "
                }
